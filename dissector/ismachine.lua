...
-- flows[flow_id] = { len, last_timestamp, avg_len }
flows = {}
...
function ismachine_protocol.dissector(buffer, pinfo, tree)
  ...
  local flow_id = src .. "," .. src_port .. "," .. dst .. "," .. dst_port .. "," .. proto
  ...
  if not flows[flow_id] then
    flows[flow_id] = {}
    flows[flow_id].len = 1,
    flows[flow_id].last_timestamp = timestamp,
    flows[flow_id].avg_len = len,
  else
    delta_time = timestamp - flows[flow_id].last_timestamp
    avg_len = (flows[flow_id].avg_len * (flows[flow_id].len / (flows[flow_id].len + 1))) + len * (1 - (flows[flow_id].len / (flows[flow_id].len + 1)))
    flows[flow_id].len = flows[flow_id].len + 1
    flows[flow_id].last_timestamp = timestamp,
    flows[flow_id].avg_len = avg_len,
  end

  local args = { src_port, dst_port, proto, delta_time, len, avg_len }
  print(dump(args))
