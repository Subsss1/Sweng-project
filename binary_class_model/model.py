...
def predict_with_probabilities(model, data):
...
test_results, test_probabilities = predict_with_probabilities(model, testing_data)

# Print results for the first 10 test samples
for i in range(10):
    print(f"Sample {i + 1}: Actual Label = {testing_labels[i]}, Predicted Label = {test_results[i]}, Probabilities = {test_probabilities[i]}")
